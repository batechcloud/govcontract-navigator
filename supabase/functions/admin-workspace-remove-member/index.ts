// Admin removes a member from a workspace. Owner cannot be removed.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (target.role === "owner") return json({ error: "Cannot remove workspace owner" }, 400);
    ...
    await admin.from("sync_audit_log").insert({
      ...
      details: { workspace_id: workspaceId, user_id: userId },
    });
